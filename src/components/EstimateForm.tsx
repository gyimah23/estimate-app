...
const EstimateForm = ({ estimate, onSave, onCancel }: EstimateFormProps) => {
  ...
  const shareViaWhatsApp = () => {
    const { materialsCost, laborCost, grandTotal } = calculateTotals();
    
    const message = `*AG ELECTRICAL ESTIMATE*\n\n` +
      `Project: ${projectTitle}\n` +
      `Client: ${clientName}\n` +
      `Date: ${new Date().toLocaleDateString()}\n\n` +
      `Materials: ${selectedCurrency.symbol}${materialsCost.toFixed(2)}\n` +
      `Labor: ${selectedCurrency.symbol}${laborCost.toFixed(2)}\n` +
      `*Total: ${selectedCurrency.symbol}${grandTotal.toFixed(2)}*\n\n` +
    ...
    toast({
      ...
      description: "WhatsApp opened with estimate details",
    });
  };
